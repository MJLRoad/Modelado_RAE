...
# ╔═╡ 3465efc2-7ae3-11ef-2462-23df6e1accd9
using PlutoUI,Random, Distributions, CairoMakie, LaTeXStrings

# ╔═╡ 3ddc6ab7-f847-40aa-adb0-b022bb99e459
md"""
# Modelo de Intercambio Aleatorio de Activos (RAE)
"""

# ╔═╡ 2ec69f68-99cc-4f0f-bd73-efce23d2d27f
md"""
## Paquetes a utilizar
"""

# ╔═╡ 95d600af-04e4-471a-a283-b898a63c643f
TableOfContents(title="Tabla de Contenido", indent=true, depth=5, aside=true)

# ╔═╡ 66c24b38-2177-4c8b-a5d0-a197df59e64e
md"""
...
# ╔═╡ 9cf71d14-243a-4c44-a900-5ab782a2bdb1
md"""
### Descripción del modelo
"""

# ╔═╡ cef0d7dc-858d-4f46-95fa-c8ed07d7568a
md"""
...
# ╔═╡ 5b11ec39-9603-4118-bfd4-f00c2e2beb1c
md"""
Algunos supuestos:
...
# ╔═╡ b361e672-fef4-46f1-af74-636a29049bcc
md"""
...
# ╔═╡ dbee63f0-2b6f-4f73-a455-e3956acd1f81
md"""
## Cómo se realizan las simulaciones
"""

# ╔═╡ 93f2cbc6-b0a6-43e4-9434-28c02c5d60f2
md"""
...
# ╔═╡ 27f22f58-1126-451f-ad6e-8259dc7c8c66
md"""
...
# ╔═╡ ed7e2163-6270-4a21-a498-cff1c029370c
md"""
...
# ╔═╡ 2b34cb9b-4f98-4408-b68d-174403e92d46
begin 
	N_exp = 5
...
# ╔═╡ 48208710-ba90-4db6-a91c-e909a1ac3915
mutable struct Agent
  money::Float64
end

# ╔═╡ 4e09bf7b-74c2-436e-b327-f8adca814032
...
# ╔═╡ fee41d77-95d9-4e12-9e7a-093a6d463ea3
sim = @time simulation(5, 6, 1, 6)

# ╔═╡ d3b919ef-c4ae-4571-881d-eab34429ad1d
begin # parametros para la curva de la ditribucion B-G
	max_val_sim = maximum(sim)
	max_val_sim_int = Int(floor(max_val_sim))
	m_values = range(0, stop = max_val_sim, length=max_val_sim_int)
	P_values = 500_000 * (1 / 10) * exp.(-m_values / 10)  # Use `exp.` for element-wise exponentiation
end

# ╔═╡ 1509472b-0f3f-4f4c-bf38-21453fae7801
begin
	...
	lines!(m_values, P_values, color=:red, label = "Distribución B-G")
	axislegend(position=:rt)
	f
end

# ╔═╡ 6a27af4d-038a-46e0-81bd-f22f11b627b1
@bind agents PlutoUI.Slider(1:6, show_value=true, default=4)

# ╔═╡ f96e36d6-d299-48d0-9999-a8228cb4bd04
@bind q_money PlutoUI.Slider(1:7, show_value=true, default=6)

# ╔═╡ 206b585d-7728-49ec-905c-1c3d15a4fc8d
@bind rules PlutoUI.Slider(1:3, show_value=true, default=2)

# ╔═╡ 6e9948b4-dbc5-4e32-a96d-edb7dd156cfd
@bind time PlutoUI.Slider(3:7, show_value=true, default=5)

# ╔═╡ 14647a4f-7aa9-4908-ba57-203f5515b25b
sim_d = @time simulation(agents, q_money, rules, time)

# ╔═╡ 8359c982-bc26-4e76-8e59-1ce8eef46e86
begin # parametros para la curva de la ditribucion B-G
	max_val_sim_d = maximum(sim_d)
	max_val_sim_d_int = Int(floor(max_val_sim_d))
	m_values_d = range(0, stop = max_val_sim_d, length=max_val_sim_d_int)
	P_values_d = (5*10^agents) * (1 / ((5*10^q_money) / (5*10^agents))) * exp.(-m_values_d / ((5*10^q_money) / (5*10^agents)))  # Use `exp.` for element-wise exponentiation
end

# ╔═╡ ef0c68a2-6142-4b2b-bd31-d03832fb211f
begin
	f_d = Figure(size = (800, 500))
	ax_d = Axis(f_d[1, 1],
	title = "Distribución del dinero con Agentes: 5x10^$agents Cantidad de dinero: 5x10^$q_money Regla: $rules y Tiempo: 2x10^$time",
    xlabel = "Cantidad de Dinero",
    ylabel = "Agentes",
	)
	stephist!(ax_d, sim_d, bins = max_val_sim_d_int, color = :black, label = "Simulación")
	lines!(m_values_d, P_values_d, color=:red, label = "Distribución B-G")
	axislegend(position=:rt)
	f_d
end

# ╔═╡ 95a9fa87-9dde-4599-b2c4-ef87c89d4573
md"""
Agentes: 5x10^$agents 

Cantidad de dinero: 5x10^$q_money 
...
manifest_format = "2.0"
project_hash = "0da8353e70099f0d5d644d3c4d2035e96149ffb0"
